...
type HealthData struct {
	...
	Systolic   int       `gorm:"not null" json:"systolic"`                // Tekanan darah sistolik (mmHg)
	Diastolic  int       `gorm:"not null" json:"diastolic"`              // Tekanan darah diastolik (mmHg)
	BloodSugar int       `gorm:"not null" json:"blood_sugar"`            // Gula darah (mg/dL)
	Weight     float64   `gorm:"not null" json:"weight"`                  // Berat badan (kg)
	HeartRate  int       `gorm:"not null" json:"heart_rate"`             // Detak jantung (bpm)
